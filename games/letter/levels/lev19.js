...
letters[rand].innerText = '0';
letters[rand2].innerText = '0';
letters[rand3].innerText = '0';
letters[rand4].innerText = '0';
let rainting = 3;
let current = 0;
    ...
    item.onclick = () => {
        if(item.innerText == '0'){
			...
			if (infoPoint.innerText == 4) {
				...
				localStorage.setItem('letLev19', rainting);
				setTimeout(() => {
...
nextLevel.onclick = () => {
    ...
    effect.ontransitionend = () => location.href = '../menu.html';
}
backLevel.onclick = () =>{
    effect.style.clipPath = 'circle(100%)';
    effect.ontransitionend = () => location.href = 'lev18.html';
};
