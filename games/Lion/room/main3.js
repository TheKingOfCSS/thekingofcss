...
let set = setInterval(() => {
    ...
    if (wrag.offsetLeft <= laser.offsetLeft + laser.offsetWidth && laser.offsetTop + laser.offsetHeight >= wrag.offsetTop && wrag.offsetLeft + wrag.offsetWidth >= laser.offsetLeft) {
        wrag.style.backgroundImage = 'url(../img/death3.png)';
        gameBool = false;
        coin += .02;
        coinText.innerText = parseInt(coin)  + ' / 30';
    }
    if (wrag.offsetLeft + wrag.offsetWidth < 0) {
        wrag.style.backgroundImage = 'url(../img/wrag3.gif)';
        gameBool = true;
    }
	if (coin >= 30) {
		...
		clearInterval(set);
		setTimeout(() => {
			localStorage.setItem('lionGame', 4);
			localStorage.setItem('lev3', 30);
