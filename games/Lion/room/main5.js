...
let set = setInterval(() => {
    ...
    if (wrag.offsetLeft <= laser.offsetLeft + laser.offsetWidth && laser.offsetTop + laser.offsetHeight >= wrag.offsetTop && wrag.offsetLeft + wrag.offsetWidth >= laser.offsetLeft) {
        wrag.style.backgroundImage = 'url(../img/death5.png)';
        gameBool = false;
        coin += .02;
        coinText.innerText = parseInt(coin)  + ' / 70';
    }
    if (wrag.offsetLeft + wrag.offsetWidth < 0) {
        wrag.style.backgroundImage = 'url(../img/wrag5.gif)';
        gameBool = true;
    }
	if (coin >= 70) {
		...
		clearInterval(set);
		setTimeout(() => {
			localStorage.setItem('lionGame', 6);
			localStorage.setItem('lev5', 70);
