...
let set = setInterval(() => {
    ...
    if (wrag.offsetLeft <= laser.offsetLeft + laser.offsetWidth && laser.offsetTop + laser.offsetHeight >= wrag.offsetTop && wrag.offsetLeft + wrag.offsetWidth >= laser.offsetLeft) {
        wrag.style.backgroundImage = 'url(../img/death4.png)';
        gameBool = false;
        coin += .03;
        coinText.innerText = parseInt(coin)  + ' / 50';
    }
    if (wrag.offsetLeft + wrag.offsetWidth < 0) {
        wrag.style.backgroundImage = 'url(../img/wrag4.gif)';
        gameBool = true;
    }
	if (coin >= 50) {
		...
		clearInterval(set);
		setTimeout(() => {
			localStorage.setItem('lionGame', 5);
			localStorage.setItem('lev4', 50);
