        ...
        if (overlaySecondText == 2) {
            ...
            setTimeout(() => {
                ...
                let timer = setInterval(() => {
                    ...
                    if (GameTimerText <= 10) {
                        const ice = document.querySelectorAll('.level1 .boxBlock .ice');
                        for (let i = 0; i < ice.length; i++) {
                            ice[i].style.opacity = 0;
                            ice[i].style.visibility = 'hidden';
                        }
                    }
                    if (boxCount == box.length) {
                        clearInterval(timer);
                        localStorage.setItem('gameLevel', 12);
                        setTimeout(() => {
...
levelsPlayButton[1].onclick = function() {
    ...
    setTimeout(() => {
        window.location.href = 'code12.html';
    }, 500);
};
